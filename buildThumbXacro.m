...
%
% All translation values have been converted from meters to millimeters.
...
% From xacro (converted to mm): origin xyz = [-11.5, 39, -15.85]
% rpy = (-1.3045e-15, 1.5708, 0)
tform1 = trvec2tform([-0.0115, 0.0390, -0.01585]*1000) * ...
         eul2tform([ -1.3045e-15, 1.5708, 0 ], 'XYZ');
...
% From xacro (converted to mm): origin xyz = [-58.25, 0, 32.25]
% rpy = (-pi, 1.5708, 0)
tform2 = trvec2tform([-0.05825, 0, 0.03225]*1000) * ...
         eul2tform([-pi, 1.5708, 0], 'XYZ');
...
% From xacro (converted to mm): origin xyz = [24.99792, -12, -24]
% rpy = (-pi/2, 0, -pi)
tform3 = trvec2tform([0.02499792, -0.012, -0.024]*1000) * ...
         eul2tform([-pi/2, 0, -pi], 'XYZ');
...
% From xacro (converted to mm): origin xyz = [-60, 0, 0]
% rpy = (0, 0, -pi)
tform4 = trvec2tform([-0.06, 0, 0]*1000) * ...
         eul2tform([0, 0, -pi], 'XYZ');
...
% From xacro (converted to mm): origin xyz = [76.1, 5.20213, -2.5]
% rpy = (1.5708, 0, -pi)
tformFingertip = trvec2tform([0.0761, 0.00520213, -0.0025]*1000) * ...
                 eul2tform([1.5708, 0, -pi], 'XYZ');
